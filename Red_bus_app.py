...
engine = create_engine('mysql+pymysql://root:admin@127.0.0.1:3306/redbus_data1')
...
data = fetch_data(initial_query)

# Handle NaN and non-numeric values
data['duration'] = data['duration'].str.extract(r'(\d+)').astype(float).fillna(0).astype(int)
data['price'] = pd.to_numeric(data['price'], errors='coerce').fillna(data['price'].mean())
data['star_rating'] = pd.to_numeric(data['star_rating'], errors='coerce').fillna(data['star_rating'].mean())
...
# Bustype Filter
bustype_options = np.append(['Anything'], data['bustype'].unique())
bustype_filter = st.multiselect('Select Bus Type:', options=bustype_options)

# Route Filter
route_options = np.append(['Anything'], data['route_name'].unique())
...
price_filter_map = {
    'Anything': (data['price'].min(), data['price'].max()),
    '0-250': (0, 250),
    '250-500': (250, 500),
    '500-1000': (500, 1000),
    '1000-1500': (1000, 1500),
    '1500+': (1500, data['price'].max())
}
...
availability_options = ['Anything'] + [str(i) for i in range(0, int(data['seats_available'].max()) + 1)]
...
duration_map = {
    'Anything': (0, data['duration'].max()),
    '0-2': (0, 2),
    '2-4': (2, 4),
    '4-6': (4, 6),
    '6+': (6, data['duration'].max())
}
...
st.write('Price vs Star Rating:')
st.line_chart(filtered_data[['price', 'star_rating']])

# Bus Type vs Seat Availability
st.write('Bus Type vs Seat Availability:')
